...
- `fields()`: group logically — ID first, timestamps last. Use `App\Nova\Fields\ID` (custom field, hidden from index by default) instead of `Laravel\Nova\Fields\ID`. Timestamps should use `->onlyOnDetail()` (never show on index). Use `->sortable()` on filterable columns.
