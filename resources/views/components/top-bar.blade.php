...
<header class="h-14 flex-shrink-0 bg-[#07090b] flex items-center gap-4 px-5 z-20 max-[640px]:px-3 max-[640px]:gap-2"
    ...
    <div class="flex items-center gap-[10px] whitespace-nowrap">
        ...
        @if($console)
            <span class="font-mono text-[10px] font-bold tracking-[0.12em] uppercase py-[3px] px-2 rounded-chip text-accent"
                  style="background: color-mix(in srgb, var(--accent) 16%, transparent); border: 1px solid color-mix(in srgb, var(--accent) 38%, transparent)">Sysadmin</span>
            <span class="font-mono text-[11px] font-medium tracking-[0.06em] uppercase text-ink-6 max-[820px]:hidden">Platform ops</span>
        @else
            <span class="text-ink-5 text-[13px]">—</span>
            <span class="font-mono text-[11px] font-medium tracking-[0.06em] uppercase text-accent max-[820px]:hidden">Quantum Crypto Bot</span>
        @endif
    </div>
    ...
    <button type="button" class="flex items-center gap-[9px] bg-transparent border border-transparent rounded-control py-[5px] pr-2 pl-[6px] cursor-pointer transition-colors duration-fast ease-out hover:bg-ink-1">
        <span class="w-[30px] h-[30px] rounded-full text-accent font-mono font-bold text-[12px] flex items-center justify-center"
              style="background: color-mix(in srgb, var(--accent) 18%, transparent)">{{ $userInitials }}</span>
