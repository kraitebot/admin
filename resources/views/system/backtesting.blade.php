    ...
    <script>
        ...
        window.btConsole = (config) => ({
            ...
            HELP_META: {
                ...
                max_daily_amplitude: { t: 'Max daily amplitude %', s: 'Largest price range recorded inside one UTC calendar day.', b: "For every **UTC calendar day**, this takes the lowest candle low and highest candle high, then calculates **(high − low) ÷ low × 100**.\n\nThe displayed value is the largest daily amplitude in the selected history window. It measures market movement only — independent of trade direction, ladder settings and simulated outcomes." },
                        ...
                        <template x-if="sampleSize > 0">
                            <div class="flex flex-col gap-4">
                                ...
                                <div class="grid grid-cols-3 gap-3 max-[640px]:grid-cols-2">
                                    ...
                                    <div class="{{ $statCard }}"><span class="{{ $statLabel }} inline-flex items-center gap-[5px]">Max daily amplitude %<x-ui.help-dot topic="max_daily_amplitude"/></span><span class="{{ $statVal }}" style="color: var(--warn)" x-text="fmtFixed(totals.max_daily_amplitude_pct)"></span><span class="{{ $statSub }}" x-text="totals.max_daily_amplitude_date ? totals.max_daily_amplitude_date + ' UTC' : 'no candle data'"></span></div>
